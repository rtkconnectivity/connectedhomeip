...
CHIP_ERROR BLEManagerImpl::_Init()
{
    ...
    err = MapBLEError(bt_matter_adapter_init(APP_MAX_LINKS));
    bt_matter_adapter_cback_register((P_CHIP_BLEMGR_CBACK)(ble_callback_dispatcher), this);
...
CHIP_ERROR BLEManagerImpl::_SetDeviceName(const char * deviceName)
{
    ...
    if (deviceName != NULL && deviceName[0] != 0)
    {
        ...
        bt_matter_adapter_device_name_set(mDeviceName, kMaxDeviceNameLength);
...
bool BLEManagerImpl::CloseConnection(BLE_CONNECTION_OBJECT conId)
{
    ...
    // Ble close function
    err = MapBLEError(bt_matter_adapter_le_disconnect(conId));
...
{
    return bt_matter_adapter_mtu_get(conId);
}
                                    ...
                                    PacketBufferHandle data)
{
    ...
    bt_matter_adapter_indication_send(conId, data->Start(), data->DataLength());
...
CHIP_ERROR BLEManagerImpl::ConfigureAdvertisingData()
{
    ...
    bt_matter_adapter_adv_config(adv_int_min, adv_int_max, advData, sizeof(advData));
...
CHIP_ERROR BLEManagerImpl::StartAdvertising()
{
    ...
    // Start advertising
    bt_matter_adapter_adv_stop();
    vTaskDelay(100);
    bt_matter_adapter_adv_start();
...
CHIP_ERROR BLEManagerImpl::HandleGapMsg(void * param, T_IO_MSG * p_gap_msg)
{
...
CHIP_ERROR BLEManagerImpl::HandleGapLEMsg(void * param, int cb_type, void * p_cb_data)
{
    CHIP_ERROR err        = CHIP_NO_ERROR;
    ...
    //     ChipLogProgress(DeviceLayer, "GAP_MSG_LE_DATA_LEN_CHANGE_INFO: conn_id %d, tx octets 0x%x, max_tx_time 0x%x",
...
int BLEManagerImpl::ble_callback_dispatcher(void * param, void * p_cb_data, int type, T_CHIP_BLEMGR_CALLBACK_TYPE callback_type)
{
    ...
    case CB_GAP_CALLBACK:
        blemgr->HandleGapLEMsg(param, type, p_cb_data);
        break;

    case CB_GAP_MSG_CALLBACK:
        blemgr->HandleGapMsg(param, (T_IO_MSG *) p_cb_data);
        break;
