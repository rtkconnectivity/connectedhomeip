...
parent_path = os.path.dirname(parent_path)
CHIP_ROOT = os.path.dirname(parent_path)
...
def gen_test_certs(chip_cert_exe: str,
                   ...
                   valid_from: str = "2024-01-01 00:00:00",
                   lifetime: str = "4294967295"):
    """
    ...
    if generate_cd:
        ...
        cmd = [chip_cert_exe, "gen-cd",
               "--key", CD_KEY_PATH,
               "--cert", CD_PATH,
               "--out", output + "/Chip-Test-CD-" + "%04X" % (vendor_id) + "-" + "%04X" % (product_id) + ".der",
               "--format-version",  "1",
    ...
    new_certificates = {
        "PAA_CERT": output + "/Chip-PAA-RTK-Cert",
        "PAA_KEY": output + "/Chip-PAA-RTK-Key",
        "PAI_CERT": output + "/Chip-PAI-RTK-" + "%04X" % (vendor_id) + "-" + "%04X" % (product_id) + "-Cert",
        "PAI_KEY": output + "/Chip-PAI-RTK-" + "%04X" % (vendor_id) + "-" + "%04X" % (product_id) + "-Key",
        "DAC_CERT": output + "/Chip-DAC-RTK-" + "%04X" % (vendor_id) + "-" + "%04X" % (product_id) + "-Cert",
        "DAC_KEY": output + "/Chip-DAC-RTK-" + "%04X" % (vendor_id) + "-" + "%04X" % (product_id) + "-Key",
    }
...
def main():
    parser = argparse.ArgumentParser(description="RTK CHIP Certificates generator")
    ...
    parser.add_argument("--valid_from", type=str, default="2024-01-01 00:00:00",
                        help=("The start date for the certificate's validity period in"
                              "<YYYY>-<MM>-<DD> [ <HH>:<MM>:<SS> ] format. Default to 2023-01-01 00:00:00"))
    parser.add_argument("--lifetime", type=str, default="4294967295",
                        help=("The lifetime for the new certificate, in whole days."))
